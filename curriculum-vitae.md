...
2009 年に大学卒業後、キヤノン株式会社に入社し、医療機器関連の Windows ネイティブアプリ(C#、WPF)の開発に従事。2017 年から 3 年間、フリーランスとして Python を中心とした toC、toB 向けの Web アプリケーションの開発に参画。2020 年から正社員として、Python(Django)、Ruby(Ruby on Rails)を使ったバックエンド、Vue.js を使ったフロントエンド、Terraform を使ったインフラなど、特定領域に限定せず開発に携わる。

一番得意とするところはバックエンドの開発で、特に DDD などのソフトウェアアーキテクチャに興味を持ち、シンプルかつ保守性の高い設計を心がけて開発を行っている。
